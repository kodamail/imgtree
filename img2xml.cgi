...
for my $line (@js)
{
    ...
    if( $line =~ /xml_tree_filename *= *\'([^']+)\' *;/ ){ $opt{xml_tree_filename} = $1; print "$opt{xml_tree_filename}\n";}
}
...
$content .= "<body>\n";
$content .= "<p>\n";
$content .= "$opt{xml_tree_filename} is successfully created!\n";
$content .= "<\/p>\n";
$content .= "<\/body>\n";
...
sub main
{
	...
	open( FH, "> $opt{xml_tree_filename}" ) 
            or die "error in img2xml.cgi: cannot open $opt{xml_tree_filename}\n";
        print FH $xml;
        close( FH );
    }
}
...
sub do_img2xml
{
    ...
    if( $img_dir eq "" )
    {
        print STDERR "error in img2xml.cgi: top directory is not specified.\n";
        exit 1;
    }
...
#    print $i . " : " . $img_files[$i] . "\n";
        ...
        for( my $j=$#gnames+1; $j<=$#gnames_now; $j++ )
        {
            ...
            $$xml .= '  ' x @gnames;
#	    if( $type{$gnames[$j]} eq "" ){ $type{$gnames[$j]} = "child_of_${gnames[$j]}"; }
#	    if( $type{$gnames[$j]} eq "" ){ $type{$gnames[$j]} = "child_of_"; }
            if( $type{$gnames[$j]} eq "" )
            {
#		if( $j < $#gnames_now ){ $type{$gnames[$j]} = "def-" . $j; }
		if( $j < $#gnames_now ){ $type{$gnames[$j]} = "def-" . ($j+1); }
#                $type{$gnames[$j]} = ${gnames[$j]};
#                $type{$gnames[$j]} =~ s|[^/]+/$||;
#                $type{$gnames[$j]} =~ s|/|_|g;
#                $type{$gnames[$j]} = "grandchild_" . $type{$gnames[$j]};
            }
